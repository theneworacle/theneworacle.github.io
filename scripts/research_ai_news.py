...
def fetch_top_ai_stories() -> list:
    """Fetches top AI stories using DuckDuckGo News search for 'top AI news'. Includes exponential backoff on rate limit."""
    print("Fetching top AI stories from DuckDuckGo News...")
    max_retries = 5
    delay = 2
    for attempt in range(max_retries):
        try:
            results = []
            with DDGS() as ddgs:
                for r in ddgs.news(keywords="top AI news", max_results=10): # Modified query
                    results.append({'title': r['title'], 'url': r['url']})
            return results
        except Exception as e:
            print(f"Error fetching DuckDuckGo top AI news (attempt {attempt+1}): {e}")
...
def save_and_set_pr_details_tool(title: str, excerpt: str, content: str, tags: List[str], sources: List[str]) -> str:
    ...
    try:
        ...
        try:
            ...
            print("YAML frontmatter validated successfully successfully.")
        ...
        if 'GITHUB_OUTPUT' in os.environ:
            with open(os.environ['GITHUB_OUTPUT'], 'a') as gh_output:
                print(f"BRANCH_NAME=automated-ai-news-research-{timestamp}", file=gh_output) # Modified branch name
                print(f"PR_TITLE=Automated AI News Research: {title_slug if title_line else 'New AI Post'}", file=gh_output) # Modified PR title
                # Use a delimiter for multi-line body
                print("PR_BODY<<EOF", file=gh_output)
                print(f"This PR contains a new AI news research post generated by the automated workflow.\n\n## Post Content:\n\n{content}", file=gh_output) # Modified PR body
            ...
            print(f"BRANCH_NAME=automated-ai-news-research-{timestamp}") # Modified branch name
            print(f"PR_TITLE=Automated AI News Research: {title_slug if title_line else 'New AI Post'}") # Modified PR title
            print(f"PR_BODY=This PR contains a new AI news research post generated by the automated workflow.\n\n## Post Content:\n\n{content}") # Modified PR body
...
lead_author_agent = Agent(
    name="ai_lead_author", # Modified agent name
    model=GEMINI_MODEL,
    description="Lead author who selects top AI stories from DuckDuckGo News, checks for duplicates against existing post excerpts, and assigns research tasks.", # Modified description
    instruction="You are the lead author for AI news. Fetch top AI stories using fetch_top_ai_stories. Get existing post excerpts using get_existing_post_excerpts. Iterate through the fetched top stories and compare their titles/summaries against the existing post excerpts to find the first story that is NOT a duplicate. Output the title and url of the selected story as a JSON string.", # Modified instruction and tool call
    tools=[fetch_top_ai_stories, get_existing_post_excerpts], # Modified tool call
    output_key="selected_story"
)

researcher_agent = Agent(
    name="ai_researcher", # Modified agent name
    model=GEMINI_MODEL,
    description="Researcher who gathers more information and social sentiment for a given AI story.", # Modified description
    instruction="You are the AI news researcher. Given a story (title and url) from the lead author, use search_news_tool, scrape_article_tool, and search_social_sentiment to gather more information from other news sources and social media. Summarize your findings for the writer agent.", # Modified instruction
    tools=[search_news_tool, scrape_article_tool, search_social_sentiment],
    output_key="research_findings"
)

writer_agent = Agent(
    name="ai_writer", # Modified agent name
    model=GEMINI_MODEL,
    description="Writer who drafts the blog post based on selected AI story details and research findings.", # Modified description
    instruction="You are the AI news writer. Given the selected story details (title, url) and research findings, write a compelling, well-structured blog post in markdown. Include title, excerpt, content, tags, and sources. Output a JSON string with these fields.", # Modified instruction
    tools=[],
    output_key="blog_post_json_string"
)

reviewer_agent = Agent(
    name="ai_reviewer", # Modified agent name
    model=GEMINI_MODEL,
    description="Reviewer who checks the AI news draft for quality and accuracy.", # Modified description
    instruction="You are the AI news reviewer. Review the blog post draft for factual accuracy, clarity, and style. Suggest improvements if needed, or approve for publishing.", # Modified instruction
    tools=[],
    output_key="reviewed_blog_post_json_string"
)

publisher_agent = Agent(
    name="ai_publisher", # Modified agent name
    model=GEMINI_MODEL,
    description="Publishes the final AI news blog post if approved.", # Modified description
    instruction="You are the AI news publisher. If the reviewer approves, publish the post using save_and_set_pr_details_tool. Otherwise, do not publish.", # Modified instruction
    tools=[save_and_set_pr_details_tool],
    output_key="publishing_status"
)

pipeline = SequentialAgent(
    name="AINewsResearchPipeline", # Modified pipeline name
    sub_agents=[lead_author_agent, researcher_agent, writer_agent, reviewer_agent, publisher_agent]
)

# Runner setup
APP_NAME = "ai_news_research_app" # Modified app name
USER_ID = "ai_news_user" # Modified user id
SESSION_ID = "ai_news_session" # Modified session id
...
# Run the pipeline
async def run_ai_news_research_pipeline(prompt: str): # Modified function name
    print("Gemini ADK Sequential Pipeline: Starting AI news research process...") # Modified print statement
...
if __name__ == "__main__":
    ...
    initial_prompt = "Find the latest top AI news and trends relevant for a blog post." # Modified prompt

    # Run the async pipeline with proper cleanup
    pipeline_ran_successfully = asyncio.run(run_ai_news_research_pipeline(initial_prompt)) # Modified function call
            ...
            if not post_path or not post_title or not post_content:
                ...
                if direct_push_to_main:
                    ...
                    try:
                        ...
                        # Commit the changes
                        commit_message = f"feat: Add automated AI news post: {post_title}" # Modified commit message
                    ...
                    branch_name = f"new-ai-post-{branch_slug}" # Modified branch name
                    pr_title = f"AI News: {post_title}" # Modified PR title
